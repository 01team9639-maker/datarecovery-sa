...
# مهلة بين الطلبات — ليست تجميلًا.
# أول تشغيل من خادم GitHub نجح (31/33)، والتالي بعده بـ13 دقيقة رجع صفرًا في
# كل طلب بينما الموقع يستجيب طبيعيًّا من مكان آخر. الفحص كان يطلق 33 طلبًا
# متتاليًا بلا فاصل، فيستفزّ حماية الفيضان في الاستضافة ويسقط نفسه.
# مراقبة تُسقط ما تراقبه تُنتج إنذارات كاذبة، وقد تُدرج خادم الفحص في الحجب.
PAUSE="${LIVE_CHECK_PAUSE:-1.5}"
code() { sleep "$PAUSE"; curl -s -o /dev/null -m 15 -w '%{http_code}' "$SITE$1"; }
...
# سقوط كل الفحوص دفعة واحدة ليس 33 عطلًا، بل تعذّر وصول: حجب أو انقطاع شبكة
# أو حدّ طلبات. تسميته «الموقع معطّل» أرسل المالك إلى الدعم بلا داعٍ من قبل.
if [ "$pass" -eq 0 ] && [ "$fail" -gt 5 ]; then
  echo
  echo "⚠️  تعذّر الوصول إلى الخادم أصلًا — لا نتيجة لهذا الفحص."
  echo "    كل طلب رجع 000: حجب لعنوان الفاحص، أو حدّ طلبات، أو انقطاع شبكة."
  echo "    تحقّق من متصفحك قبل افتراض عطل، وأعد التشغيل بعد قليل."
  exit 2
fi
